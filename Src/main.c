...
int main(void)
{
	...
	/*
		Включенеи тактирования портов B и C, скотороыми мы работаем в этом коде. Каждая периферия тактируется (получает тактове сигналы от источника) отдельно
	...
	объявление этой структуры. Там много всего очень страшного и я просто объясню, что происходит. Регастр это физическое место памяти микроконтроллера объемом
	32 бита, то есть 4 байта (это справедливо именно для STM32 микроконтроллеров и может отличать у других) соответственно все регистры, описанные в документации
	...
	в документации и мы можем в наглядном виде предстваить, что и куда записываем.
...
//	GPIOB->MODER &= GPIO_MODER_MODE3_Msk | GPIO_MODER_MODE4_Msk;
	...
	// Данны алгоритм зажигает второй светодиод на плате, если нажата крайняя левая кнопка и тушит его, если она отжата
